...
module.exports = class {
    ...
    http(req, res, next = () => res.end('err')) {
        ...
        req.pPath = req.url.replace(this.prefix.slice(0, -1), '')
        req.pathname = req.pPath.split('#')[0].split('?')[0];
        
        if (req.pathname == '/client_hook' || req.pathname == '/client_hook/') return res.end(fs.readFileSync(__dirname + '/window.js', 'utf-8'));
    
        try {new URL(this.proxifyRequestURL(req.pPath, true))} catch {return res.end('URL Parse Error')};
    
        var proxyURL = {
            href: this.proxifyRequestURL(req.pPath, true),
            origin: this.proxifyRequestURL(req.pPath, true).split('/').splice(0, 3).join('/'),
            hostname: this.proxifyRequestURL(req.pPath, true).split('/').splice(0, 3).slice(2).join('/')
        },
            proxify = {},
            isBlocked = false,
        ...
        if (!req.pPath.startsWith(`/_${btoa(proxyURL.origin)}_/`)) return (res.writeHead(308, { location: this.prefix + `_${btoa(proxyURL.origin)}_/`}), res.end(''));
