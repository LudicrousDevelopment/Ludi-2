...
https.on('request', (req, res) => {
  ...
  if (req.url.startsWith(alloyprefix)) {
    return Alloy.http(req, res)
  }
  if (req.url.startsWith('/prox')) {
    return res.writeHead(301, {location:alloyprefix+Alloy.proxifyRequestURL(atob(req.query.url))}).end('')
  }
  ...
  if (req.url.split('?')[0].split('#')[0].startsWith('/christmas-countdown/')) {
    if (req.url.includes('.js')) var contentType = 'application/javascript';
    else if (req.url.includes('.css')) var contentType = 'text/css';
