...
https.on('request', (req, res) => {
  req.query = {}
  req.url.split('?').map(e=>e.split('&')).map(e => e.includes('=')?req.query[e.split('=')[0]]=e.split('=')[1]:null)
  ...
  if (req.url.startsWith('/prox')) {
    return res.writeHead(301, {location:alloyprefix+Alloy.proxifyRequestURL(req.query.url)})
  }
